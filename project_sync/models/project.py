...
class ProjectInherit(models.Model):
    ...
    def create_sync_queue(self, res):
        ...
        if data_to_sync:
            sync_config.create_queue('create', res.id, data_to_sync, res.write_date)
    ...
    def write_sync_queue(self, res, vals):
        ...
        if data_to_sync:
            sync_config.create_queue('update', res.id, data_to_sync, res.write_date)

    def delete_sync_queue(self, res, ):
        sync_obj = self.sudo().env['synchronization.data.mapping']
        sync_config = sync_obj.get_by_model_name(self._name)
        sync_config.create_queue('delete', res.id, {}, fields.Datetime.now())
